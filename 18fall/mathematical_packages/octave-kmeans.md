...
The k-means clustering algorithm is given a set of points, an integer `k` and splits point into `k`
sets in such a way that points of one sets visually form a group (a cluster).
For example, points from the image below may be naturally split into three clusters:

![3 clusters](../../17fall/mathematical-packages/clusters1.png)

Formally, you should implement the function `k_means`, that gets a point matrix as an input (ponts are represented by rows),
an integer `k`, and returns a column matrix with indexes of clusters from 1 to `k`.
The height of the result matrix is the sae as a height of the initial matrix.
...
The function `k_means_generate_clusters(3)` generates clusters similar to that shown on the first image, but sometimes
clusters may intersect, and the result will be like that:

![3 intersecting clusters](../../17fall/mathematical-packages/clusters2.png)

This is normal, it is interesting to run clusterization also in this situation. 
