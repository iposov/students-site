...
2. Реализуйте функцию `filterGerenator :: Random a -> (a -> Boolean) -> Random a`, эта функция генерирует случайные значения до тех пор, пока не выполнится условие. Например, `filterGenerator (randomInRange 1 10) even` создаёт генератор, который генерирует четные числа от 1 до 10.
...
4. Сгенерируйте «много» пар чисел ($K$ штук) от $-N$ до $N$, где $N = 1000$. Это координаты точек в квадрате. Посчитайте, какая часть из этих точек попала в круг с радиусом $N$ и с центром в $0$, т.е. для скольких точек выполняется ($x^2 + y^2 <= N^2$). Если таких точек $L$, можно оценить число $\pi \approx \frac{4L}{N}$.
