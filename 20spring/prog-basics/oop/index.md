...
разделы создадут для пример новые классы.

## Ключевое слово `final`

У слова `final` много применений, мы рассмотрим применения с локальными
переменными и полями класса. Слово `final` при определении локальных
переменных и полей класса означает, что значение нельзя изменять после присваивания.
...
Посмотрим это на синтетическом примере, чтобы не портить класс `Student`:
...
public class A {
    ...
    A() {
        ...
        // можно выбрать, что присвоить, но присвоить только один раз
        if (x > 0)
            z = 5;
        else
            z = -5;
    ...
    A(int y) {
        this.y = y;
        z = -5; // в этом конструкторе тоже надо что-то присвоить
    }   
