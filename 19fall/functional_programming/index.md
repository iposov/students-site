...
В этом разделе я помечу __*__ важные задачи.  
...
поддеревьев). И еще она принимает на вход дерево: `treeFold :: b -> (a -> b -> b -> b) -> Tree b -> b`.
...
1. __*__ Дерево поиска. Представьте, что у нас есть правило для того, какие значения хранятся
...
1. __*__ Реализуйте функцию поиска минимального элемента в дереве поиска.
